...
AddPackage doas-sudo-shim # A shim for the sudo command that utilizes doas
AddPackage dosfstools # DOS filesystem utilities
AddPackage evolution-data-server # Unified contacts, tasks and calendar backend
...
AddPackage game-devices-udev # Udev rules for controllers
...
AddPackage gnome-browser-connector # Native browser connector for integration with extensions.gnome.org
...
AddPackage logmein-hamachi # A zero-configuration VPN service
...
AddPackage nautilus-open-any-terminal # context-menu entry for opening other terminal in nautilus
...
AddPackage toolbox # Unprivileged development environment
